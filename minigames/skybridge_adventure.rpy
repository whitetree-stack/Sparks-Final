...
init python in beacon_quest:
    ...
    # ----------------------------------------------------------------
    # TILESET LOADING (placeholder for future sprite support)
    # ----------------------------------------------------------------
    def load_image(path):
    ...
    def load_tileset():
        """Placeholder - currently using procedural rendering."""
        global TILESET_LOADED
        TILESET_LOADED = True
        print("Using procedural dungeon rendering")
        return True
    ...
    def draw_procedural_door(surf, px, py, tile_size, direction, time_ms):
        """Draw a subtle door indicator - just a small arrow."""
        center_x = px + tile_size // 2
        center_y = py + tile_size // 2
        # Subtle pulsing arrow only
        glow = 0.7 + 0.3 * math.sin(time_ms * 0.004)
        alpha = int(180 * glow)
        arrow_color = (200, 220, 255, alpha)
        arrow_surf = pygame.Surface((tile_size, tile_size), pygame.SRCALPHA)
        ac = tile_size // 2  # arrow center
        if direction == 'N':
            pygame.draw.polygon(arrow_surf, arrow_color, [(ac, 8), (ac - 10, 24), (ac + 10, 24)])
        elif direction == 'S':
            pygame.draw.polygon(arrow_surf, arrow_color, [(ac, tile_size - 8), (ac - 10, tile_size - 24), (ac + 10, tile_size - 24)])
        elif direction == 'E':
            pygame.draw.polygon(arrow_surf, arrow_color, [(tile_size - 8, ac), (tile_size - 24, ac - 10), (tile_size - 24, ac + 10)])
        elif direction == 'W':
            pygame.draw.polygon(arrow_surf, arrow_color, [(8, ac), (24, ac - 10), (24, ac + 10)])
        surf.blit(arrow_surf, (px, py))
    ...
    def draw_procedural_slime(surf, screen_x, screen_y, width, height, anim_phase, is_dying, death_timer):
        ...
        pygame.draw.ellipse(slime_surf, (*body_color[:3], int(80 * alpha / 255)), (0, 0, body_w + 10, body_h + 10))
        pygame.draw.ellipse(slime_surf, (*body_color[:3], alpha), (5, 5, body_w, body_h))
        pygame.draw.ellipse(slime_surf, (*highlight_color[:3], int(alpha * 0.8)), (8, 8, body_w - 15, body_h // 2))
    ...
    def draw_procedural_vampire(surf, screen_x, screen_y, width, height, anim_phase, facing, is_dying, death_timer):
        ...
        pygame.draw.ellipse(body_surf, (*body_color[:3], alpha), (15, 10, width - 30, height - 25))
        surf.blit(body_surf, (screen_x, screen_y + int(hover)))
        head_y = screen_y + 5 + hover
        pygame.draw.circle(surf, (*body_color[:3], alpha) if alpha == 255 else body_color, (int(screen_x + width // 2), int(head_y + 10)), 12)
        if alpha > 100:
            eye_y = head_y + 8
            ...
            pygame.draw.polygon(surf, (255, 255, 255), [(screen_x + width // 2 - 4, fang_y), (screen_x + width // 2 - 2, fang_y + 5), (screen_x + width // 2 - 6, fang_y)])
            pygame.draw.polygon(surf, (255, 255, 255), [(screen_x + width // 2 + 4, fang_y), (screen_x + width // 2 + 2, fang_y + 5), (screen_x + width // 2 + 6, fang_y)])
    ...
    def draw_procedural_orc(surf, screen_x, screen_y, width, height, anim_phase, is_dying, death_timer):
        ...
        if alpha > 100:
            ...
            pygame.draw.polygon(surf, (230, 220, 200), [(screen_x + width // 2 - 12, tusk_y), (screen_x + width // 2 - 8, tusk_y + 8), (screen_x + width // 2 - 14, tusk_y + 3)])
            pygame.draw.polygon(surf, (230, 220, 200), [(screen_x + width // 2 + 12, tusk_y), (screen_x + width // 2 + 8, tusk_y + 8), (screen_x + width // 2 + 14, tusk_y + 3)])
    ...
    def draw_procedural_enemy(surf, screen_x, screen_y, width, height, anim_phase, hit_flash):
        ...
        color = (255, 200, 200) if hit_flash > 0 else (80, 50, 100)
    ...
    class BeaconQuestGame:
        ...
        def __init__(self):
            ...
            # Initialize tileset (procedural for now)
            load_tileset()
        ...
        def draw_map(self, surf, time_ms):
            """Draw the game map using procedural rendering with camera offset."""
            room = self.current_room
            ...
            for y in range(start_y, end_y):
                for x in range(start_x, end_x):
                    ...
                    # DOORS - floor with subtle arrow indicator
